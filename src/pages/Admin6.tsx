...
import { ArrowLeft, Moon, Sun, Loader2, ExternalLink, ShieldOff, CheckCircle } from 'lucide-react';
...
const Admin6 = () => {
    ...
    const { data: applications, isLoading: isApplicationsLoading, isError: isApplicationsError, error: applicationsError } = useQuery({
        ...
        queryFn: async () => {
            // Only fetch if the user is identified as an admin
            if (!isAdmin) {
                // If not an admin, return an empty array or throw an error to prevent unauthorized data fetching
                // For a more robust solution, you'd handle this on the backend with RLS.
                // However, client-side check provides immediate feedback.
        ...
        // Query is enabled only if profile data has loaded AND user is determined to be an admin
        enabled: !isProfileLoading && isAdmin,
        retry: false, // Do not retry on failures related to access denied
    });
    ...
    const rawUserPlan = profile?.plan?.toLowerCase() || 'free'; // This might not be available if profile only fetches 'role'
