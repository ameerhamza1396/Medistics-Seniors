...
import { Link } from 'react-router-dom'; // Keep Link for any internal navigation if explicitly needed
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { ExternalLink, ShieldOff, CheckCircle, XCircle } from 'lucide-react'; // Removed ArrowLeft, Moon, Sun, Loader2 as they are now in AdminHeader/AdminLockout
...
const Admin6 = () => {
  ...
  const { data: applications, isLoading: isApplicationsLoading, isError: isApplicationsError, error: applicationsError } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .order('created_at', { ascending: false }); // Order by newest applications first
    ...
    enabled: true,
    retry: false, // Do not retry on failures that might indicate permission issues (though AdminLockout should prevent these)
  });

  // Define plan color schemes (used if displaying user plan, though AdminHeader handles admin badge)
  const planColors = {
      'free': {
  ...
  // The original plan-related variables are no longer needed here as AdminHeader handles the badge.
  ...
  const renderApplicationCard = (app: any) => ( // Use 'any' for now, or define a proper interface for ambassador_applications
      <Card key={app.id} className="bg-white dark:bg-gray-800 border border-blue-200 dark:border-blue-800 shadow-md hover:shadow-lg transition-shadow duration-200">
          ...
          <CardContent className="text-sm text-gray-600 dark:text-gray-300 space-y-2">
              ...
              <p><strong>College:</strong> {app.college_name}</p>
              ...
              <div className="pt-3 border-t border-gray-200 dark:border-gray-700 mt-3 space-y-2">
                  ...
                  <ul className="space-y-1">
                      ...
                      <li>
                          <a href={app.college_id_card_url} target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 flex items-center">
                              College ID Card <ExternalLink className="ml-1 h-3 w-3" />
                          </a>
                      </li>
                      <li>
                          <a href={app.recent_marksheet_url} target="_blank" rel="noopener noreferrer" className="text-purple-600 hover:text-purple-700 dark:text-purple-400 dark:hover:text-purple-300 flex items-center">
                              Recent Marksheet <ExternalLink className="ml-1 h-3 w-3" />
                          </a>
                      </li>
                  </ul>
              </div>
  ...
  return (
    <AdminLockout>
      <div className="min-h-screen w-full bg-white dark:bg-gray-900">
        {/* Use the shared AdminHeader component */}
        <AdminHeader userEmail={user?.email} />

        <div className="container mx-auto px-4 lg:px-8 py-8 max-w-7xl">
          <div className="text-center mb-8 animate-fade-in">
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4">
              📊 Ambassador Applications Admin Panel
            </h1>
          ...
          {isApplicationsLoading ? (
            ...
            <div className="text-center p-8 bg-blue-50 dark:bg-blue-900/20 rounded-lg max-w-2xl mx-auto">
              ...
              <p className="text-blue-600 dark:text-blue-300">There are currently no ambassador applications to display.</p>
            </div>
          ) : (
