...
import { ChevronDown, ChevronUp, Search as SearchIcon, AlertTriangle, XCircle } from 'lucide-react';
...
export default function Admin4() {
  ...
  const isAlphabeticCorrectAnswer = (mcq: MCQ) => {
    if (!mcq.correct_answer) return false;
    const normalizedCorrectAnswer = mcq.correct_answer.trim().toUpperCase();
    return ['A', 'B', 'C', 'D', 'E'].includes(normalizedCorrectAnswer);
  };

  const getFixableByAlphabeticMapping = (mcq: MCQ) => {
    if (!mcq.correct_answer) return false;
  ...
  const getOtherProblematic = (mcq: MCQ) => {
    ...
    const normalizedCorrectAnswer = mcq.correct_answer.trim().toUpperCase();
    const isAlphabeticBased = ['A', 'B', 'C', 'D', 'E'].includes(normalizedCorrectAnswer);

    if (isAlphabeticBased && getFixableByAlphabeticMapping(mcq)) {
      return false;
    }
  ...
  const getProblematic = (mcq: MCQ) => {
    return getFixableByAlphabeticMapping(mcq) || getOtherProblematic(mcq);
  };

  const totalFixableByAlphabetic = mcqs.filter(getFixableByAlphabeticMapping).length;
  const totalOtherProblematic = mcqs.filter(getOtherProblematic).length;
  const totalProblematic = totalFixableByAlphabetic + totalOtherProblematic;
  ...
  const handleFixAlphabeticProblematic = async () => {
    setFixing(true);
    const fixableMCQs = mcqs.filter(getFixableByAlphabeticMapping);
    let fixedCount = 0;
    let failedCount = 0;
        ...
        <div className="container mx-auto px-4 lg:px-8 py-8 max-w-7xl">
          ...
          <p className="text-gray-900 dark:text-gray-100">
            ...
            <span className="text-yellow-600 dark:text-yellow-400 font-semibold">
              <AlertTriangle className="inline-block h-4 w-4 mr-1 mb-0.5" /> {totalFixableByAlphabetic} A-E fixable
            </span>,{' '}
          ...
          {totalFixableByAlphabetic > 0 && (
            <button
              onClick={handleFixAlphabeticProblematic}
              className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded disabled:opacity-50"
              disabled={fixing}
            >
              {fixing ? 'Fixing...' : `Fix A/B/C/D/E Correct Answers (${totalFixableByAlphabetic})`}
            </button>
          )}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4 mt-4">
            {subjects.map((subject, index) => {
              ...
              const subjectFixableByAlphabetic = subjectMCQs.filter(getFixableByAlphabeticMapping).length;
                  ...
                  <CardContent className="py-4">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="flex items-center space-x-2">
                        {subjectFixableByAlphabetic > 0 && (
                          <span className="flex items-center text-yellow-300 dark:text-yellow-200">
                            <AlertTriangle className="h-4 w-4 mr-1" /> {subjectFixableByAlphabetic}
                          </span>
                        )}
                        ...
                        {(subjectFixableByAlphabetic === 0 && subjectOtherProblematic === 0) && (
            ...
            <div className="space-y-3 mt-4">
              {chapters
                ...
                .map((chapter) => {
                  ...
                  const chapterFixableByAlphabetic = chapterMCQs.filter(getFixableByAlphabeticMapping).length;
                    ...
                    >
                      <CardContent className="py-4">
                        <div className="flex justify-between items-center">
                          ...
                          <span className="flex items-center space-x-2">
                            {chapterFixableByAlphabetic > 0 && (
                              <span className="flex items-center text-yellow-600 dark:text-yellow-400">
                                <AlertTriangle className="h-4 w-4 mr-1" /> {chapterFixableByAlphabetic}
                              </span>
                            )}
                            ...
                            {(chapterFixableByAlphabetic === 0 && chapterOtherProblematic === 0) && (
            ...
            <div className="space-y-3 mt-4">
              {mcqs
                ...
                .map((mcq) => (
                  <Card
                    key={mcq.id}
                    className={`border ${getFixableByAlphabeticMapping(mcq) ? 'border-orange-500 dark:border-orange-700' : 'border-red-400 dark:border-red-700'} bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100`}
                  ...
                  >
                    <CardContent className="space-y-2">
                      ...
                      {mcq.correct_answer === null || !mcq.options?.includes(mcq.correct_answer) || getFixableByAlphabeticMapping(mcq) ? (
                      ...
                      {getFixableByAlphabeticMapping(mcq) && (
                        <p className="text-sm text-orange-700 dark:text-orange-400">
                          💡 This can be fixed automatically (A/B/C/D/E mapping).
                        </p>
                      )}
                        ...
                        <ul className="list-disc list-inside text-sm text-gray-700 dark:text-gray-300">
                          {mcq.options.map((opt, idx) => (
                            <li key={idx} className={mcq.correct_answer && mcq.correct_answer === opt ? 'font-bold text-green-700 dark:text-green-400' : ''}>
                              ...
                              {mcq.correct_answer && isAlphabeticCorrectAnswer(mcq) && String.fromCharCode(65 + idx) === mcq.correct_answer.toUpperCase() && (
                                <span className="ml-2 text-orange-500 dark:text-orange-300">(Current A/B/C/D/E mapping)</span>
                              )}
                            </li>
                          ))}
                        </ul>
